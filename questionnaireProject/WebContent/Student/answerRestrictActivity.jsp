...
<body>
	<form action="answerRestrictActivity" method="post">
	<fieldset>
	<%
	...
	if(session.getAttribute("问卷题目数")!=null)
	{
		session.setAttribute("问卷题目数", session.getAttribute("问卷题目数"));
	}else
	{
	...
	int i=Integer.parseInt(session.getAttribute("题目编号-1").toString());
	System.out.println("题目编号-1："+i);
	String TID=((SELECTQUESTION)list01.get(i)).getTID();
	session.setAttribute("题号", TID);
	String TIDNO=((SELECTQUESTION)list01.get(i)).getTIDNO();
	
	String sql1="SELECT * FROM QUESTION WHERE TID='"+TID+"'";
	System.out.println("学员查询题目表sql语句："+sql1);
	//题目表list1.size()题目个数
	List<Object>list1=quObj.selectRecord(sql1);
	
	
		out.println((i+1)+"、"+((question)list1.get(0)).getStem()+"<br>");
		String type=((question)list1.get(0)).getType();
		session.setAttribute("题目类型", type);
	...
	%>
	<div align="center"><input type="submit" value="下一题">
	<input type="button" onclick="window.location.href='../Login/welcome2.jsp'" value="取消"></div>
	<input type="hidden" name="answerRestrictActivity" value="answerRestrictActivity">
	</fieldset>
	</form>
</body>
</html>