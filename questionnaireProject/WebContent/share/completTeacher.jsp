...
function validate_form(thisform)
	{
		with (thisform)
  	{
  		if (validate_required(teaName,"请输入姓名")==false||
  				validate_required(teaMail,"请输入邮箱")==false||
  				validate_required(teaTelphone,"请输入联系号码")==false)
   		 {
  			return false
  		}
  		}
...
<body>
<%
		String sql="SELECT * FROM TEACHER WHERE TEAID='"+session.getAttribute("用户名").toString()+"'";
		if(shareFunction.selectTeacherInformation(sql)!=null)
		{
			out.println("<script>alert('信息已经完善');location.href='../Login/welcome1.jsp';</script>");
		}
%>
	<div align="center">
		<form action="completTeacher" onsubmit="return validate_form(this)" method="post">
		<fieldset>
		...
		<fieldset>
			姓名
				<input type="text"  name=teaName placeholder="填写姓名"  required  style="height:40px;width:449px;background-color:#E0EEE0">
		</fieldset>
		<fieldset>
			邮箱
				<input type="email"  name=teaMail placeholder="填写邮箱"  required  style="height:40px;width:449px;background-color:#E0EEE0">
		</fieldset>
		<fieldset>
			电话
				<input type="text" pattern="(\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$" 
				name=teaTelphone title="请输入正确的号码" placeholder="填写号码"  required  style="height:40px;width:449px;background-color:#E0EEE0">
		</fieldset>
					<div align="center"><input type="submit" value="提交">
					<input type="reset" value="重置"></div>
				
			<input type="hidden" name="completeTeacher" value="completeTeacher">
			</fieldset>
		</form>
	</div>
</body>
